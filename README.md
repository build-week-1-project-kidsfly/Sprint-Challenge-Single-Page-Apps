...
> Answer: Client-side routing handles routing via the Javascript internally while the page is being loaded. As the user clicks on a link the request to the server is prevented but the url changes. The adjustmnet of the url results in a changed state of the application. The changed state ultimately renders a different view of the web page. 
